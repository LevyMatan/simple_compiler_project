...
size_t vector_total_size(struct vector *vector) { return vector->count * vector->esize; }
...
void vector_resize_for_index(struct vector *vector, int start_index, int total_elements) {
  ...
  vector->data = realloc(
      vector->data, ((start_index + total_elements + VECTOR_ELEMENT_INCREMENT) * vector->esize));
  assert(vector->data);
...
void *vector_at(struct vector *vector, int index) { return vector->data + (index * vector->esize); }

void vector_set_peek_pointer(struct vector *vector, int index) { vector->pindex = index; }
...
void vector_unset_flag(struct vector *vector, int flag) { vector->flags &= ~flag; }
...
size_t vector_elements_left(struct vector *vector, int index) { return vector->count - index; }

int vector_elements_until_end(struct vector *vector, int index) { return vector->count - index; }
