...
char* get_token_flags_name(void* p_type) {
    int type = *(int*)p_type;
    if (type < 0 || type >= sizeof(token_flags_dict) / sizeof(token_flags_dict[0])) {
        return "Invalid type";
    }
    return token_flags_dict[type];
}
...
getter_func_t g_a_getters[] = {
    get_trace_level_name,   get_trace_status_name,
    get_parser_status_name, get_LEXICAL_ANALSYS_STATAUS_name,
    get_token_type_name,    get_token_flags_name,
    get_number_type_name,   get_node_type_name,
...
int typename_to_idx(const char* type_name) {
    ...
    if (strcmp(type_name, "parser_status") == 0) return 2;
    if (strcmp(type_name, "LEXICAL_ANALSYS_STATAUS") == 0) return 3;
    if (strcmp(type_name, "token_type") == 0) return 4;
    if (strcmp(type_name, "token_flags") == 0) return 5;
    if (strcmp(type_name, "number_type") == 0) return 6;
    if (strcmp(type_name, "node_type") == 0) return 7;
    if (strcmp(type_name, "int") == 0) return 8;
    if (strcmp(type_name, "float") == 0) return 9;
    if (strcmp(type_name, "double") == 0) return 10;
    if (strcmp(type_name, "char") == 0) return 11;
    if (strcmp(type_name, "short") == 0) return 12;
    if (strcmp(type_name, "long") == 0) return 13;
    if (strcmp(type_name, "long_long") == 0) return 14;
