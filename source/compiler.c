...
void compiler_error(compile_process_t *p_process, const char *format, ...)
{
    ...
    fprintf(stderr, "%s:%d:%d: error: ", p_process->cfile.abs_path, p_process->s_pos.line, p_process->s_pos.col);
...
void compiler_warning(compile_process_t *p_process, const char *format, ...)
{
    va_list args;
    va_start(args, format);
    fprintf(stderr, "%s:%d:%d: warnining: ", p_process->cfile.abs_path, p_process->s_pos.line, p_process->s_pos.col);
    vfprintf(stderr, format, args);
    va_end(args);
}
